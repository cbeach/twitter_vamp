...
class CluBot(irc.IRCClient):
    ...
    def privmsg(self, user, channel, msg):
        ...
        if msg == '!reload':
            ...
            rebuild.latestClass(cmds.CluCmds)
            rebuild.updateInstance(self.cmds)
            self.cmds._update_cmdlist()
            rebuild.rebuild(voice)
            self.msg(channel, 'reloaded and ready to go')
        else:
            reply = self.cmds.dispatch(user, msg)
...
class CluBotFactory(protocol.ClientFactory):
...
class CluBotContextFactory(ssl.ClientContextFactory):
